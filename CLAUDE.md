...
Apple Sign In is currently a Luminality-only feature; `fundbright-web` and `nutripod-web` do not consume this gem.

After publishing a new version via `/publish-gem`, roll it out with the workspace-level `/rollout-gem standard_id-apple [<version>]` skill. Keep this list in sync with the consumer matrix in `<workspace>/.claude/skills/rollout-gem/SKILL.md`.
