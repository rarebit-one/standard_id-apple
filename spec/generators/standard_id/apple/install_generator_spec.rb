...
require "generators/standard_id/apple/install/install_generator"

RSpec.describe StandardId::Apple::Generators::InstallGenerator do
  let(:destination_root) { @destination_root }
  let(:initializer_path) { File.join(destination_root, "config/initializers/standard_id_apple.rb") }

  before do
    @destination_root = Dir.mktmpdir("standard_id_apple_generator")
  ...
  it "is registered under the standard_id:apple:install namespace" do
    expect(described_class.namespace).to eq("standard_id:apple:install")
  end
  ...
  it "configures every apple field in the social scope" do
    run_generator
  ...
  it "never emits the unqualified form" do
    ...
    expect(File.read(initializer_path)).not_to match(/^\s*config\.apple_/)
  end
  ...
  describe "idempotence" do
    ...
    it "overwrites with --force" do
      ...
      expect(File.read(initializer_path)).to include("config.social.apple_client_id")
    end
