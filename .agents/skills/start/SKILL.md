---
name: start
description: "Start working on GitHub Issues in rarebit-one/standard_id-apple. Use when the user says 'start working on', 'pick up issue', 'work on #42', 'work on standard_id-apple-42', 'start #42', '/start', or wants to begin development on a planned issue. Handles context gathering, branch creation, and in-progress signaling."
---

# Start Skill

Begin working on issues from **rarebit-one/standard_id-apple** (GitHub Issues) with proper setup: gather context, create branches, signal in-progress, and track progress.

> **Planning system:** This gem's planning lives in **its own GitHub Issues** (`rarebit-one/standard_id-apple`). Because code and issues live in the **same repo**, a PR can auto-close its issue with a `Closes #NN` keyword in the PR body.

## Prerequisites

This skill uses the `gh` CLI against `rarebit-one/standard_id-apple`. If `gh` is unavailable or unauthenticated, the skill will warn and offer to proceed with git-only setup (branch/worktree creation without the in-progress signal). Check with `gh auth status` if calls fail.
...
```
/start <issue-numbers...>        # Start specific issues (e.g., /start 42 43, /start standard_id-apple-42, /start #42)
/start --mine                    # Show my assigned open issues
/start --backlog                 # Show open, unassigned issues
```

Accepted identifier forms: `42`, `#42`, `standard_id-apple-42`, or a full issue URL. All normalize to the issue number.
...
```bash
gh api repos/rarebit-one/standard_id-apple/issues/<n> \
...
gh api repos/rarebit-one/standard_id-apple/issues/<n>/comments \
...
```bash
gh issue list -R rarebit-one/standard_id-apple --assignee @me --state open --limit 10
```

**If `--backlog` flag (optionally with `--label <name>`):**

```bash
gh issue list -R rarebit-one/standard_id-apple --state open --search "no:assignee" --limit 10
```
...
```bash
gh api repos/rarebit-one/standard_id-apple/issues/<referenced-n> --jq '{number, title, state}'
```
...
```bash
gh issue edit <n> -R rarebit-one/standard_id-apple --add-assignee @me
...
> **Optional label:** if the repo uses an `in progress` (or similar) label, add it with `gh issue edit <n> -R rarebit-one/standard_id-apple --add-label "in progress"`. Skip if the repo has no such label.
...
📋 Issue: <title>
🔗 https://github.com/rarebit-one/standard_id-apple/issues/42
...
| Flag | Description |
|------|-------------|
| `--mine` | List my assigned open issues in rarebit-one/standard_id-apple |
...
| Issue not found | Verify the number; confirm the repo is `rarebit-one/standard_id-apple` |
